...
struct CpuidBit {
  ...
  uint8_t reg;
  uint32_t bit;
  bool cpuid::Features::*flag;
  VendorId* vendor_id;
};

CpuidBit cpuid_bits[] = {
    {1, 2, 21, &cpuid::Features::x2apic},
    {0x40000001, 0, 6, &cpuid::Features::kvm_pv_eoi, &kvm_vendor_id},
    {0x40000001, 0, 3, &cpuid::Features::kvm_clocksource2,
     &kvm_vendor_id}};

constexpr size_t nr_cpuid_bits = sizeof(cpuid_bits) / sizeof(CpuidBit);
}  // namespace

cpuid::Features cpuid::features;

cpuid::Result cpuid::Cpuid(uint32_t leaf) {
  Result r;
